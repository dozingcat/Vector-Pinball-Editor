...
public class Field7Delegate extends BaseFieldDelegate {
    ...
    // Mapped to star indices defined below.
    ...
    @Override public void tick(Field field, long nanos) {
        ...
        /*
        rightAscension += 0.005;
        if (rightAscension > TAU) {
            rightAscension -= TAU;
        }
        */
        double ra = constellations[constellationIndex].centerRaRadians;
        double dec = constellations[constellationIndex].centerDecRadians;
        projectVisibleStars(catalog, ra, dec, 0.5, projection);
        field.setShapes(shapesFromProjection(projection, 9.4, 13, 4.5/.5, 0.07));
    }
    ...
    List<Shape> shapesFromProjection(
        ...
        for (int i = 0; i < proj.size(); i++) {
            ...
            int baseColor = constellations[constellationIndex].starIndices.contains(proj.indices.get(i)) ?
                    consColor : noConsColor;
            int color = Color.withAlpha(baseColor, alpha);
            double rmul = (mag <= 0) ? 1.5 : (mag >= 4) ? 0.5 : 1.0;
    ...
    static class Constellation {
        ...
        static Constellation withSegments(StarCatalog catalog, String name, int... segments) {
            ...
            for (int index : starIndices) {
                double ra = catalog.raRadians[index];
                double dec = catalog.decRadians[index];
                double dist = Math.hypot(ra - avgRa, dec - avgDec);
